...
const Navbar = () => {
  ...
  const displaystyle1 = display ? "absolute w-full bg-[#D7D7D7] left-[0] top-[71px] opacity-100 transition-all ease-in-out duration-1000" : "transition-all ease-in-out w-full duration-1000 absolute lg:w-1/2 left-[0] top-[-351px] opacity-0 md:block md:opacity-100"
  const displaystyle2 = "text-center font-bold md:static"
  return (
    <nav className={`z-50 fixed top-0 w-full 2xl:px-[100px] lg:px-[50px]  ${navbarBg ? "bg-[#D7D7D7]" : "bg-transparent"}`}>
      <div className="flex items-center lg:justify-normal justify-between p-3 font-[Montserrat]">
        ...
        <div className={`${displaystyle1} ${displaystyle2}`}>
          <div className={`flex flex-col justify-between md:flex-row text-[17px] font-bold ${!navbarBg && "lg:text-[white]"}`}>
